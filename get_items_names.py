...
for current_position in range(0, total_items + 50, 50):
    all_items_get = requests.get("https://steamcommunity.com/market/search/render/?start="+str(current_position)+"&count=100&search_descriptions=0&sort_column=default&sort_dir=desc&appid="+game_id+"&norender=1&count=5000", cookies=cookie)
    print(f'Items {str(current_position)} out of {str(total_items)}, code: {str(all_items_get.status_code)}')
    all_items = all_items_get.content
    all_items = json.loads(all_items)
...
with open(game_id+'item_names.txt', 'wb') as file:
