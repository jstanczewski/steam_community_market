...
for current_item in all_items_names:
    ...
    print(
        f"{str(current_run)} out of {str(len(all_items_names))}, code: {str(item.status_code)}"
    )
    current_run += 1
    item = item.content
        ...
        if not item_price_data:
            ...
            for current_day in range(len(item_date) - 1, 1, -1):
                if item_date[current_day] == item_date[current_day - 1]:
                    item_prices[current_day - 1] = np.mean(
            ...
            current_item_dict = {
                ...
                "initial": item_price_initial,
                "days_on_market": days_on_market,
                "price_increase": price_increase,
                "price_avg": item_price_avg,
                "max_price": max_price,
                "max_idx": max_idx,
                "min_price": min_price,
                "min_idx": min_idx,
